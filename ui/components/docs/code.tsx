	...
	return (
		...
		>
			<div className="flex items-center space-x-2">
				<div className="w-4 h-4 bg-vite-500 rounded-full" />
				<div className="w-4 h-4 bg-vite-500 rounded-full" />
				<div className="w-4 h-4 bg-vite-500 rounded-full" />
			</div>
			<div
			...
			<button className="absolute right-2 top-2 w-8 h-8">
				<Copy />
			</button>
			<div className="w-full mt-6 text-vite-200 overflow-x-scroll overflow-hidden">
				<code className="microlight">
					<pre className="pb-5 microlight" dangerouslySetInnerHTML={{ __html: props.children }} />
				</code>
			</div>
		</div>
	);
